...
export class AvisosFormComponent implements OnInit, OnDestroy {
  ...
  // Tracks the editor innerHTML at all times so we can use it on save
  // instead of calling setValue() (which would trigger writeValue → setContents → DOM rebuild)
  ...
  form = this.fb.group({
    ...
    resumo:   ['', [Validators.maxLength(400)]],
  ...
  onEditorInit(event: { editor: any }): void {
    ...
    // Capture current editor state (sanitised by Quill from the loaded DB value)
    const initialHtml = this.quillEditor.root.innerHTML;
    this.editorHtml = initialHtml === '<p><br></p>' ? '' : initialHtml;

    // Keep editorHtml in sync on every user edit.
    // We do NOT call setValue() here because that would trigger writeValue() →
    // setContents() → full DOM rebuild → cursor lost + resizeImg ref invalidated.
    ...
    // Click on image inside editor → show overlay
    ...
    // Keep overlay aligned when the user scrolls inside the editor
    this.quillEditor.root.addEventListener('scroll', () => {
      if (this.resizeImg) this.updateOverlayPosition();
    });

    // Hide overlay on click anywhere outside the editor wrapper
  ...
  private buildResizeOverlay(): void {
    ...
    // 6 handles: 4 corners + left/right edges
  ...
  private onDragEnd(): void {
    ...
    // setAttribute ensures the style is in outerHTML (not only in .style object)
    this.resizeImg.setAttribute('style', style);

    // Update Quill's Delta with 'api' source so further text edits keep the size,
    // without triggering PrimeNG's text-change handler (avoids getSemanticHTML() override)
    try {
    ...
    // Capture AFTER updateContents so innerHTML reflects the final Quill DOM state
    const html = this.quillEditor.root.innerHTML;
    this.editorHtml = html === '<p><br></p>' ? '' : html;
    ...
    this.svc.getById(id).subscribe(item => {
      if (item) {
        ...
        this.form.patchValue({
          ...
          resumo: item.resumo, corpo: item.corpo,
          urgente: item.urgente, ativo: item.ativo, destaque: item.destaque
        });
      }
    });
  }
  ...
  salvar(): void {
    ...
    // editorHtml is set from innerHTML (preserves inline styles like image widths).
    // We avoid calling setValue() during editing to prevent Quill DOM rebuilds,
    // so we override corpo here at submission time instead.
